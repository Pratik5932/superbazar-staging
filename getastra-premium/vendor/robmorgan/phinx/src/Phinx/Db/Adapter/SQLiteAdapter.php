...
use AstraPrefixed\Cake\Database\Driver\Sqlite as SqliteDriver;
...
use AstraPrefixed\Phinx\Util\Expression;
...
class SQLiteAdapter extends PdoAdapter
{
    ...
    protected static $supportedColumnTypes = [self::PHINX_TYPE_BIG_INTEGER => 'biginteger', self::PHINX_TYPE_BINARY => 'binary_blob', self::PHINX_TYPE_BLOB => 'blob', self::PHINX_TYPE_BOOLEAN => 'boolean_integer', self::PHINX_TYPE_CHAR => 'char', self::PHINX_TYPE_DATE => 'date_text', self::PHINX_TYPE_DATETIME => 'datetime_text', self::PHINX_TYPE_DOUBLE => 'double', self::PHINX_TYPE_FLOAT => 'float', self::PHINX_TYPE_INTEGER => 'integer', self::PHINX_TYPE_JSON => 'json_text', self::PHINX_TYPE_JSONB => 'jsonb_text', self::PHINX_TYPE_SMALL_INTEGER => 'smallinteger', self::PHINX_TYPE_STRING => 'varchar', self::PHINX_TYPE_TEXT => 'text', self::PHINX_TYPE_TIME => 'time_text', self::PHINX_TYPE_UUID => 'uuid_text', self::PHINX_TYPE_TIMESTAMP => 'timestamp_text', self::PHINX_TYPE_VARBINARY => 'varbinary_blob'];
    // list of aliases of supported column types
    protected static $supportedColumnTypeAliases = ['varchar' => self::PHINX_TYPE_STRING, 'tinyint' => self::PHINX_TYPE_SMALL_INTEGER, 'tinyinteger' => self::PHINX_TYPE_SMALL_INTEGER, 'smallint' => self::PHINX_TYPE_SMALL_INTEGER, 'int' => self::PHINX_TYPE_INTEGER, 'mediumint' => self::PHINX_TYPE_INTEGER, 'mediuminteger' => self::PHINX_TYPE_INTEGER, 'bigint' => self::PHINX_TYPE_BIG_INTEGER, 'tinytext' => self::PHINX_TYPE_TEXT, 'mediumtext' => self::PHINX_TYPE_TEXT, 'longtext' => self::PHINX_TYPE_TEXT, 'tinyblob' => self::PHINX_TYPE_BLOB, 'mediumblob' => self::PHINX_TYPE_BLOB, 'longblob' => self::PHINX_TYPE_BLOB, 'real' => self::PHINX_TYPE_FLOAT];
    // list of known but unsupported Phinx column types
    protected static $unsupportedColumnTypes = [self::PHINX_TYPE_BIT, self::PHINX_TYPE_CIDR, self::PHINX_TYPE_DECIMAL, self::PHINX_TYPE_ENUM, self::PHINX_TYPE_FILESTREAM, self::PHINX_TYPE_GEOMETRY, self::PHINX_TYPE_INET, self::PHINX_TYPE_INTERVAL, self::PHINX_TYPE_LINESTRING, self::PHINX_TYPE_MACADDR, self::PHINX_TYPE_POINT, self::PHINX_TYPE_POLYGON, self::PHINX_TYPE_SET];
    protected $definitionsWithLimits = ['CHAR', 'CHARACTER', 'VARCHAR', 'VARYING CHARACTER', 'NCHAR', 'NATIVE CHARACTER', 'NVARCHAR'];
    ...
    public function connect()
    {
        if ($this->connection === null) {
            if (!\class_exists('PDO') || !\in_array('sqlite', PDO::getAvailableDrivers(), \true)) {
    ...
    {
        $info = $this->getSchemaName($tableName, \true);
        ...
        foreach ($schemata as $schema) {
            ...
            foreach ($rows as $row) {
                if (\strtolower($row['name']) === $table) {
                    return ['schema' => $schema, 'table' => $row['name'], 'exists' => \true];
                }
            }
        }
    ...
    protected function getChangePrimaryKeyInstructions(Table $table, $newColumns)
    {
        ...
        if (!empty($newColumns)) {
            ...
            $instructions->merge($this->getAddPrimaryKeyInstructions($table, $newColumns));
        }
        return $instructions;
    }
    /**
    ...
    protected function getRenameTableInstructions($tableName, $newTableName)
    {
        ...
        $sql = \sprintf('ALTER TABLE %s RENAME TO %s', $this->quoteTableName($tableName), $this->quoteTableName($newTableName));
    ...
    public function truncateTable($tableName)
    {
        ...
        $this->execute(\sprintf('DELETE FROM %s.%s', $this->quoteColumnName($info['schema']), $this->quoteColumnName($info['table'])));
        // assuming no error occurred, reset the autoincrement (if any)
        if ($this->hasTable($info['schema'] . '.sqlite_sequence')) {
            $this->execute(\sprintf('DELETE FROM %s.%s where name  = %s', $this->quoteColumnName($info['schema']), 'sqlite_sequence', $this->quoteString($info['table'])));
        }
    }
    /**
...
PCRE_PATTERN;
    ...
    public function getColumns($tableName)
    {
        ...
        foreach ($rows as $columnInfo) {
            ...
            $column->setName($columnInfo['name'])->setNull($columnInfo['notnull'] !== '1')->setDefault($default)->setType($type['name'])->setLimit($type['limit'])->setScale($type['scale'])->setIdentity($columnInfo['name'] === $identity);
            $columns[] = $column;
        }
        return $columns;
    }
    ...
    {
        $alter = \sprintf('ALTER TABLE %s ADD COLUMN %s %s', $this->quoteTableName($table->getName()), $this->quoteColumnName($column->getName()), $this->getColumnSqlDefinition($column));
    ...
    {
        $sql = \sprintf('INSERT INTO %s(%s) SELECT %s FROM %s', $this->quoteTableName($tableName), \implode(', ', $writeColumns), \implode(', ', $selectColumns), $this->quoteTableName($tmpTableName));
        $this->execute($sql);
    }
    /**
    ...
    protected function beginAlterByCopyTable($tableName)
    {
        ...
        $instructions->addPostStep(function ($state) use($tableName) {
            ...
            $createSQL = \preg_replace("/^CREATE TABLE .* \\(/Ui", '', $createSQL);
    ...
    protected function getRenameColumnInstructions($tableName, $columnName, $newColumnName)
    {
        ...
        $instructions->addPostStep(function ($state) use($columnName, $newColumnName) {
            $sql = \str_replace($this->quoteColumnName($columnName), $this->quoteColumnName($newColumnName), $state['createSQL']);
            $this->execute($sql);
            return $state;
        });
    ...
    protected function getChangeColumnInstructions($tableName, $columnName, Column $newColumn)
    {
        ...
        $instructions->addPostStep(function ($state) use($columnName, $newColumn) {
            $sql = \preg_replace(\sprintf("/%s(?:\\/\\*.*?\\*\\/|\\([^)]+\\)|'[^']*?'|[^,])+([,)])/", $this->quoteColumnName($columnName)), \sprintf('%s %s$1', $this->quoteColumnName($newColumn->getName()), $this->getColumnSqlDefinition($newColumn)), $state['createSQL'], 1);
            $this->execute($sql);
            return $state;
        });
        $instructions->addPostStep(function ($state) use($columnName, $newColumnName, $tableName) {
    ...
    protected function getDropColumnInstructions($tableName, $columnName)
    {
        ...
        $instructions->addPostStep(function ($state) use($columnName) {
            $sql = \preg_replace(\sprintf("/%s\\s%s.*(,\\s(?!')|\\)\$)/U", \preg_quote($this->quoteColumnName($columnName)), \preg_quote($state['columnType'])), '', $state['createSQL']);
            if (\substr($sql, -2) === ', ') {
                $sql = \substr($sql, 0, -2) . ')';
            }
            $this->execute($sql);
    ...
    protected function getAddIndexInstructions(Table $table, Index $index)
    {
        ...
        $sql = \sprintf('CREATE %s ON %s (%s)', $this->getIndexSqlDefinition($table, $index), $this->quoteTableName($table->getName()), $indexColumns);
        return new AlterInstructions([], [$sql]);
    }
    /**
     * @inheritDoc
    ...
    protected function getDropIndexByColumnsInstructions($tableName, $columns)
    {
        ...
        foreach ($indexNames as $indexName) {
            if (\strpos($indexName, 'sqlite_autoindex_') !== 0) {
                $instructions->addPostStep(\sprintf('DROP INDEX %s%s', $schema, $this->quoteColumnName($indexName)));
            }
        }
        return $instructions;
    }
    ...
    protected function getDropForeignKeyByColumnsInstructions($tableName, $columns)
    {
        ...
        $instructions->addPostStep(function ($state) use($columns) {
            ...
            foreach ($columns as $columnName) {
                $search = \sprintf("/,[^,]*\\(%s(?:,`?(.*)`?)?\\) REFERENCES[^,]*\\([^\\)]*\\)[^,)]*/", $this->quoteColumnName($columnName));
                $sql = \preg_replace($search, '', $state['createSQL'], 1);
            }
            if ($sql) {
                $this->execute($sql);
        ...
        $instructions->addPostStep(function ($state) use($columns) {
            ...
            if (!empty($diff)) {
                throw new InvalidArgumentException(\sprintf('The specified columns don\'t exist: ' . \implode(', ', $diff)));
            }
            return $newState + $state;
        });
        return $this->copyAndDropTmpTable($instructions, $tableName);
    ...
    public function getPhinxType($sqlTypeDef)
    {
        ...
        return ['name' => $name, 'limit' => $limit, 'scale' => $scale];
    }
    /**
     * {@inheritDoc}
     *
