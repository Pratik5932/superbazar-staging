...
namespace Superbazaar\CustomWork\Controller\Cart;
    ...
    { 
        ...
        if (is_array($orderItemIds)) {
            ...
            foreach ($itemsCollection as $item) {
                try {
                    // check home page post code and shipping post code
                    ...
                    if($custId){
                        ...
                        $zipcode = $item->getOrder()->getShippingAddress()->getData("postcode");
                        if($homepage_postcode != $zipcode){
                            $this->messageManager->addErrorMessage(__('"Shipping Postcode" entered at home page  is '.$homepage_postcode.' whereas this re order / recently ordered item(s) are from post code '.$zipcode.'. Hence the product could not be added to cart. Kindly search for that product(s) again and continue shopping.'));
                            return $this->_goBack();
                        }
                    }
