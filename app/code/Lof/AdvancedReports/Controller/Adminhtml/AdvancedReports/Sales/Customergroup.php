...
class Customergroup extends \Lof\AdvancedReports\Controller\Adminhtml\AdvancedReports\Sales
{
    /**
    ...
    {    
        ...
        $this->_initAction()->_setActiveMenu(
            'Lof_AdvancedReports::salesbycustomergroup'
        )->_addBreadcrumb(
            __('Sales Customer Group'),
            __('Sales Customer Group')
        );
        $this->_registry->register('report_type', "customergroup"); 
        $this->_registry->register('header_text', __("Sales Customer Group"));
        $this->_view->getPage()->getConfig()->getTitle()->prepend(__('Sales Customer Group'));   
        $chartBlock = $this->_view->getLayout()->getBlock('report.chart');
        $gridBlock = $this->_view->getLayout()->getBlock('adminhtml_advancedreport_sales_customergroup.grid');  
        $filterFormBlock = $this->_view->getLayout()->getBlock('grid.filter.form');
        $this->_initReportAction([$gridBlock, $filterFormBlock,$chartBlock], 'customergroup'); 
    ...
    protected function _isAllowed()
    {
       ...
       /* if (!$this->getData('is_valid') && !$this->getData('local_valid')) {
        ...
        return $this->_authorization->isAllowed('Lof_AdvancedReports::salesbycustomergroup');
    } 
