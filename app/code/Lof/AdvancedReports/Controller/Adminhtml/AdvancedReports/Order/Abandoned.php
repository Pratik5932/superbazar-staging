...
class Abandoned extends \Lof\AdvancedReports\Controller\Adminhtml\AdvancedReports\Order
{
    /**
    ...
    {    
        ...
        $this->_initAction()->_setActiveMenu(
            'Lof_AdvancedReports::abandoned'
        )->_addBreadcrumb(
            __('Abandoned Carts'),
            __('Abandoned Carts')
        );
        $this->_registry->register('report_type', "abandoned"); 
        $this->_registry->register('header_text', __("Abandoned Carts"));
        $this->_view->getPage()->getConfig()->getTitle()->prepend(__('Abandoned Carts'));   
        $chartBlock = $this->_view->getLayout()->getBlock('report.chart');
        $gridBlock = $this->_view->getLayout()->getBlock('adminhtml_advancedreport_order_abandoned.grid');  
        $filterFormBlock = $this->_view->getLayout()->getBlock('grid.filter.form');
        $this->_initReportAction([$gridBlock, $filterFormBlock, $chartBlock], 'abandoned'); 
    ...
    protected function _isAllowed()
    {
       ...
       /* if (!$this->getData('is_valid') && !$this->getData('local_valid')) {
        ...
        return $this->_authorization->isAllowed('Lof_AdvancedReports::abandoned');
    }
