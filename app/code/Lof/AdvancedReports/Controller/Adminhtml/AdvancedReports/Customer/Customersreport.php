...
class Customersreport extends \Lof\AdvancedReports\Controller\Adminhtml\AdvancedReports\Customer
{
    /**
    ...
    {    
        ...
        $this->_initAction()->_setActiveMenu(
            'Lof_AdvancedReports::customersreport'
        )->_addBreadcrumb(
            __('Customer Report'),
            __('Customer Report')
        );
        $this->_registry->register('report_type', "customersreport"); 
        $this->_registry->register('header_text', __("Customer Report"));
        $this->_view->getPage()->getConfig()->getTitle()->prepend(__('Customer Report'));   
        
        $gridBlock = $this->_view->getLayout()->getBlock('adminhtml_advancedreport_customer_customersreport.grid');  
        $filterFormBlock = $this->_view->getLayout()->getBlock('grid.filter.form');
        $this->_initReportAction([$gridBlock, $filterFormBlock], 'customersreport'); 
    ...
    protected function _isAllowed()
    {
     ...
     /* if (!$this->getData('is_valid') && !$this->getData('local_valid')) {
        ...
        return $this->_authorization->isAllowed('Lof_AdvancedReports::customersreport');
    }
