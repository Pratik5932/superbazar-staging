...
class Customerscountry extends \Lof\AdvancedReports\Controller\Adminhtml\AdvancedReports\Customer
{
    /**
    ...
    {    
        ...
        $this->_initAction()->_setActiveMenu(
            'Lof_AdvancedReports::customerscountry'
        )->_addBreadcrumb(
            __('Customers by Country'),
            __('Customers by Country')
        );
        $this->_registry->register('report_type', "customerscountry"); 
        $this->_registry->register('header_text', __("Customers by Country"));
        $this->_view->getPage()->getConfig()->getTitle()->prepend(__('Customers by Country'));   
        
        $gridBlock = $this->_view->getLayout()->getBlock('adminhtml_advancedreport_customer_customerscountry.grid');  
        $filterFormBlock = $this->_view->getLayout()->getBlock('grid.filter.form');
        $this->_initReportAction([$gridBlock, $filterFormBlock], 'customerscountry'); 
    ...
    protected function _isAllowed()
    {
      ...
      /*if (!$this->getData('is_valid') && !$this->getData('local_valid')) {
        ...
        return $this->_authorization->isAllowed('Lof_AdvancedReports::customerscountry');
    }
