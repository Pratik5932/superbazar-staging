...
class Ajaxgetproduct extends Action
{
	...
	public function execute()
	{
		...
		if ($queryText) {
			...
			/*if ($searchFulltext == '1') {
				...
				// Webkul Code
					$webkulHelper = $this->_objectManager->get("\Webkul\MpHyperLocal\Helper\Data");
					$savedAddress = $webkulHelper->getSavedAddress();
					$allowedProList = [];
					
					#print_r($savedAddress);exit;
					if ($savedAddress) {
						$sellerIds = $webkulHelper->getNearestSellers();
						$allowedProList = $webkulHelper->getNearestProducts($sellerIds);
					}
				// Webkul code 	
				$collection = $this->_layerResolver->get()
					...
					->addAttributeToFilter('entity_id', ['in' => $allowedProList]) // Webkul 
 ...
 #echo $priceHelper->currency($_product->getPrice(), true, false);exit;
