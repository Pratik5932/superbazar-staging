...
class HomePageData extends AbstractCatalog
{
    ...
    public function execute()
    {
        try {
            ...
            if ($this->storeId == 0) {
                $this->storeId = $this->websiteManager->create()->load($this->websiteId)->getDefaultGroup()
                    ->getDefaultStoreId();
            ...
            $currencies = [];
            $this->returnArray["allowedCurrencies"] = $this->storeInterface->getStore()
                ->getAvailableCurrencyCodes(false);
            foreach ($this->storeInterface->getStore()->getAvailableCurrencyCodes(false) as $code) {
                $currencies[] =[
                    "label" => $this->currencyInterface->getCurrency($code)->getSymbol()." ".
                        $this->currencyInterface->getCurrency($code)->getName(),
                    "code" => $code
                ];
            }
            if ($this->currency == "") {
            ...
            $this->returnArray["defaultCurrency"] = $this->helper->getConfigData("currency/options/default");
            $this->returnArray["tabCategoryView"] = $this->helper->getConfigData(
                "mobikul/tab_category_view/tab_category_view"
            );
            $this->returnArray["launcherIconType"] = $this->helper->getConfigData(
                "mobikul/launcher_icon_config/launcher_icon_type"
            );
            
            $this->returnArray["darkAppButtonColor"] = $this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_app_button_color"
            );
            $this->returnArray["darkAppThemeColor"] = $this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_app_theme_color"
            );
            $this->returnArray["darkAppThemeTextColor"] = $this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_app_theme_text_color"
            );
            $this->returnArray["darkButtonTextColor"] = $this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_button_text_color"
            );
            // $this->returnArray["darkAppBackgroundColor"] = $this->helper->getConfigData(
            //     "mobikul/dark_mode_config/dark_app_background_color"
            // );
            $filePath = $this->helper->getUrl("media")."mobikul/logo/".$this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_logo"
            );
            $dominantColorPath = $this->helper->getBaseMediaDirPath()."mobikul/logo/".$this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_logo"
            );
            $darkSplashImage = $this->helper->getConfigData(
                "mobikul/dark_mode_config/dark_splash_image"
            );
            if ($darkSplashImage) {
                $basePath = $this->baseDir.DS.'mobikul/splash/'.$darkSplashImage;
                $splashHeight = $this->width * 2;
                $splashNewPath = $this->width."x".$splashHeight;
                $newPath = $this->baseDir.DS."mobikul/splash/$splashNewPath/".$darkSplashImage;
                $this->helperCatalog->resizeNCache($basePath, $newPath, $this->width, $splashHeight);
                $newSplashUrl = $this->helper->getUrl("media")."mobikul".DS."splash".DS.$this->width."x".
                    $splashHeight.DS.$darkSplashImage;
            } else {
                $newSplashUrl = "";
            }
            $this->returnArray["darkAppLogo"] = $filePath;
            $this->returnArray["darkSplashImage"] = $newSplashUrl;
            $this->returnArray["darkAppLogoDominantColor"] = $this->helper->getDominantColor($dominantColorPath);

            $this->returnArray["appButtonColor"] = $this->helper->getConfigData(
                "mobikul/light_mode_config/light_app_button_color"
            );
            $this->returnArray["appThemeColor"] = $this->helper->getConfigData(
                "mobikul/light_mode_config/light_app_theme_color"
            );
            $this->returnArray["appThemeTextColor"] = $this->helper->getConfigData(
                "mobikul/light_mode_config/light_app_theme_text_color"
            );
            $this->returnArray["buttonTextColor"] = $this->helper->getConfigData(
                "mobikul/light_mode_config/light_button_text_color"
            );
            // $this->returnArray["appBackgroundColor"] = $this->helper->getConfigData(
            //     "mobikul/light_mode_config/light_app_background_color"
            // );
            $filePath = $this->helper->getUrl("media")."mobikul/logo/".$this->helper->getConfigData(
                "mobikul/light_mode_config/light_logo"
            );
            $dominantColorPath = $this->helper->getBaseMediaDirPath()."mobikul/logo/".$this->helper->getConfigData(
                "mobikul/light_mode_config/light_logo"
            );
            $lightSplashImage = $this->helper->getConfigData(
                "mobikul/light_mode_config/light_splash_image"
            );
            if ($lightSplashImage) {
                $basePath = $this->baseDir.DS.'mobikul/splash/'.$lightSplashImage;
                $splashHeight = $this->width * 2;
                $splashNewPath = $this->width."x".$splashHeight;
                $newPath = $this->baseDir.DS."mobikul/splash/$splashNewPath/".$lightSplashImage;
                $this->helperCatalog->resizeNCache($basePath, $newPath, $this->width, $splashHeight);
                $newSplashUrl = $this->helper->getUrl("media")."mobikul".DS."splash".DS.$this->width."x".
                    $splashHeight.DS.$lightSplashImage;
            } else {
                $newSplashUrl = "";
            }
            $this->returnArray["splashImage"] = $newSplashUrl;
            $this->returnArray["appLogo"] = $filePath;
            $this->returnArray["appLogoDominantColor"] = $this->helper->getDominantColor($dominantColorPath);
            ...
            $this->returnArray["allowAndroidDownload"] = (bool)$this->helper->getConfigData(
                "mobikul/appdownload/allowAndroid"
            );
            ...
            $this->returnArray["inMaintenance"] = true;
    ...
    public function verifyRequest()
    {
        if ($this->getRequest()->getMethod() == "GET") {
            ...
            if ($this->websiteId == 0 && $this->storeId == 0) {
                throw new \BadMethodCallException(__("Invalid Website Id"));
            }
            if (!$this->customerId && $this->customerToken != "") {
                ...
                foreach ($carouselImageColelction as $each) {
                    ...
                    if ($this->fileDriver->isFile($basePath)) {
                        $newPath = $this->baseDir.DS."mobikulresized".DS.$this->bannerWidth."x".
                            $this->height.DS.$each->getFilename();
    ...
    protected function getFeaturedCategories()
    {
        ...
        foreach ($featuredCategoryCollection as $eachCategory) {
            ...
            $oneCategory = [];
            if ($this->fileDriver->isFile($basePath)) {
                $newPath = $this->baseDir.DS."mobikulresized".DS.$this->iconWidth."x".
                ...
                $newUrl = $this->helper->getUrl("media")."mobikulresized".DS.$this->iconWidth."x".
                    $this->iconHeight.DS.$eachCategory->getFilename();
    ...
    protected function getFeaturedDeals()
    {
        ...
        $carousel["label"] = __("Featured Products")->__toString();
    ...
    protected function getNewDeals()
    {
        ...
        $carousel["type"] = "product";
        $carousel["label"] = __("New Products")->__toString();
        $carousel["productList"] = $productList;
        if (count($carousel["productList"])) {
            $this->returnArray["carousel"][] = $carousel;
        }
    ...
    protected function getHotDeals()
    {
        ...
        $carousel["type"] = "product";
        $carousel["label"] = __("Hot Deals")->__toString();
        $carousel["productList"] = $productList;
        if (count($carousel["productList"])) {
            $this->returnArray["carousel"][] = $carousel;
        }
    ...
    {
        $rootId = $this->storeInterface->getStore($this->storeId)->getRootCategoryId();
    ...
    public function getCategoryChildren($categories)
    {
        ...
        foreach ($categories as $key => $category) {
            if (array_key_exists($category["category_id"], $categoryImages)) {
                ...
                $category["thumbnailDominantColor"] = $categoryImages[
            ...
            if ($collection->getSize() > 0) {
                foreach ($collection as $value) {
                    ...
                    if ($value->getProfile() != "") {
                        ...
                        if ($this->fileDriver->isFile($basePath)) {
                            $newPath = $this->baseDir.DS."mobikulresized".DS.$this->iconWidth."x".
                                $this->iconHeight.DS."customerpicture".DS.$this->customerId.DS.$value->getProfile();
    ...
    private function getDataFromUrl($url)
    {
        ...
        $sql = "$queryType * from ".$tableName." where request_path IN (:".
            implode(", :", array_flip($pathBind)).") AND store_id IN(0,".$this->storeId.")";
    ...
    protected function getShortOrder()
    {
        $count = 1;
        $arr = [];
