...
class Index extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $this->stockItem = $objectManager->create(\Magento\CatalogInventory\Model\Stock\Item::class);
        ...
        try {
            ...
            if (!isset($params['token'])) {
                ...
                $nvps = [
                    ...
                    "VERSION" => "72.0",
                    "RETURNURL" => $this->_url->getUrl('mobikulhttp/index/return',['quoteId'=>$quoteId]),
                ...
                $rest = [
                    ...
                    "redirect_urls" => [
                        "return_url" => $this->_url->getUrl('mobikulhttp/index/return',["quoteId"=>$quoteId]),
                ...
                foreach ($cartdata as $key => $values) {
                    ...
                    foreach ($values as $info) {
                        ...
                        if ($info['number'] != "tax") {
                            $product = $productRepository->getById($info['number']);
                            if ($product->getTypeId() == "configurable") {
                                $productTypeInstance = $product->getTypeInstance();
                                $usedProducts = $productTypeInstance->getUsedProducts($product);
                                foreach ($usedProducts  as $child) {
                                    if (strip_tags($product->getSku()) == $child->getSku()) {
                                        if (!$child->isAvailable()) {
                                            $this->helper->logDataInLogger("LocalizedException Controller_index_index execute : Product is out of stock.");
                                            $this->messageManager->addError(__("Some of %1 product is out of stock.", $child->getName()));
                                ...
                                $stockData = $stockItemRepository->getStockQty($info['number'], $storeId);
                                $backOrderProduct = $productRepoInterface->getById($info['number']);
                                $data = $backOrderProduct->getExtensionAttributes()->getStockItem()->getBackorders();
                                if (!$product->isAvailable() || ($stockData < $info['qty'] && $data != 2)) {
                                    $this->helper->logDataInLogger("LocalizedException Controller_index_index execute : Product is out of stock.");
                    ...
                    if (isset($finalShipping[$key])
                        ...
                        $adminship = false;
                        $qty=1;
                        $shippingamount = $finalShipping[$key]['amount'];
                        if($finalShipping[$key]['amount'] > 0){
                        	 $shippingamount = $finalShipping[$key]['amount'] - $finalShipping[1]['amount'];
                        }
                        ...
                        $nvps["L_PAYMENTREQUEST_".$i."_AMT".$itemc] = round($shippingamount, 2);
               ...
               /* code start */ 
            	   $helper = $objectManager->create('\Magecomp\Paymentfee\Helper\Data');
                  if($helper->isEnabled()){
                     $k= $itemc + 1;
                     $qty = 1;
                     $description = $order->getShippingAddress()->getMcPaymentfeeDescription();
                     if(!$description){
                     		$description = 'Payment fee';
                     }
                     
                     if($order->getShippingAddress()->getMcPaymentfeeDescription()){
                            
		             $nvps["L_PAYMENTREQUEST_0_NAME".$k] = $description;
		             $nvps["L_PAYMENTREQUEST_0_QTY".$k] = $qty;
		             $nvps["L_PAYMENTREQUEST_0_AMT".$k] = round($order->getShippingAddress()->getMcPaymentfeeAmount(), 2);
		            
		            
		             $rest["transactions"][$i]["item_list"]["items"][] = [
		                    "sku" => 'paymentfee_sku',
		                    "name" => $description,
		                    "description" => 'paymentfee_sku',
		                    "quantity" => $qty,
		                    "price" => round($order->getShippingAddress()->getMcPaymentfeeAmount(), 2),
		                    "currency" => $order->getBaseCurrencyCode(),
		                ];
		                
		             $price += floatval(round($order->getShippingAddress()->getMcPaymentfeeAmount(), 2));
                     }
                   }	
                /* code end */ 
