...
class ReturnAction extends \Magento\Framework\App\Action\Action
{
            ...
            if (isset($params['PayerID']) && isset($params['token'])) {
                ...
                if ($resultGetExpress
                    ...
                    if ($resultDoExpress) {
                        ...
                        $this->configMethod = "mppaypalexpresscheckout";
                        $order = $this->place($this->_initToken());
                        ...
                        return $resultRedirect->setPath('checkout/onepage/success',['incrementId'=>$order->getIncrementId()]);
                    } else {
                        $this->messageManager->addError(
    ...
    private function _getQuote()
    {
        try {
            ...
            if (!$this->quote->getId()) {
                ...
                $collection = $quoteFactory->create()->getCollection()->addFieldToFilter('entity_id', $quoteId)->getLastItem();
                $this->quote = $collection;
            }
            return $this->quote;
        } catch (\Exception $e) {
    ...
    private function doExpressCheckout($data)
    {
        try {
            ...
            foreach ($cartdata as $key => $values) {
                ...
                #mail("er.bharatmali@gmail.com","payopal",$paypalid);
    ...
    {
        $this->orderSender = $this->_objectManager->create(\Magento\Sales\Model\Order\Email\Sender\OrderSender::class);
        $this->_checkoutSession = $this->_objectManager->create(\Magento\Checkout\Model\Session::class);
        if ($shippingMethodCode) {
            $this->updateShippingMethod($shippingMethodCode);
        }

        $this->quote->setCheckoutMethod(\Magento\Checkout\Model\Type\Onepage::METHOD_CUSTOMER);

        $this->ignoreAddressValidation();

        $this->quote->setPaymentMethod('mppaypalexpresscheckout');
        $this->quote->setInventoryProcessed(false);
        $this->quote->getPayment()->importData(['method' => 'mppaypalexpresscheckout']);
        $this->quote->collectTotals();
        $this->quote->save();
        $this->quoteManagement = $this->_objectManager->create(\Magento\Quote\Model\QuoteManagement::class);
