...
class PlaceOrder extends AbstractCheckout
{
    public function execute()
    {
        try {
            ...
            # mail("er.bharatmali@gmail.com","execute","asdsad");
            if (!$this->isPaypalPayment || $this->incrementId == "") {

                # mail("er.bharatmali@gmail.com","inpayumas","asdsad");
                ...
                */
                ...
                $quote->setShippingArrivalDate($this->shippingArrivalDate);
                $quote->setShippingArrivalTimeslot($this->deliveryTimeSlot);
                $quote->setShippingArrivalComments($this->shippingArrivalComments);
                ...
                if ($order->getPayment()->getMethodInstance()->getCode() == "mpstripe") {
                    $order->setOrderApprovalStatus(1)->save();
                }


                $order->setShippingArrivalDate($this->shippingArrivalDate);
                $order->setShippingArrivalTimeslot($this->deliveryTimeSlot);
                $order->setShippingArrivalComments($this->shippingArrivalComments);
                if ($order) {
                ...
                $this->helper->log([$this->incrementId], "logResponse", $this->wholeData);
                $order = $this->orderFactory->create()->loadByIncrementId($this->incrementId);
                $order->setState(\Magento\Sales\Model\Order::STATE_PROCESSING)
                ->setStatus(\Magento\Sales\Model\Order::STATE_PROCESSING)
                ->save();
                $order->setOrderApprovalStatus(1)->save();
                if ($order->getPayment()->getMethodInstance()->getCode() == "mppaypalexpresscheckout") {
                    $order->setOrderApprovalStatus(1)->save();
                }
                ...
                $quote = $this->_objectManager->create(\Magento\Quote\Model\QuoteFactory::class)->create()->load($quoteID);
                ...
                /*$order->setShippingArrivalDate($this->shippingArrivalDate)->save();
                $order->setShippingArrivalTimeslot($this->deliveryTimeSlot)->save();
                $order->setShippingArrivalComments($this->shippingArrivalComments)->save();*/
                
                $this->saveMobikulOrder($order);
                mail("er.bharatmali@gmail.com","Paypal Order place",$order->getId());
                if(!$customer->getId())
                    $this->returnArray["showCreateAccountLink"] = true;
                $this->returnArray["email"] = $userEmail;
                if($this->salesHelper->canReorder($order->getId()))
                    $this->returnArray["canReorder"] = true;
                $this->savePurchasePointDetail($order);
                $this->returnArray["orderId"]     = $order->getId();
                $this->returnArray["success"]     = true;
                $this->returnArray["incrementId"] = $this->incrementId;
            }
    ...
    */
    ...
    public function getdeductPrice($quote)
    {
    ...
    */
    public function verifyRequest()
    {
        if ($this->getRequest()->getMethod() == "POST" && $this->wholeData) {
            ...
            $this->isPaypalPayment = $this->wholeData['isPaypalPayment'] ?? false;
            $this->customerId = $this->helper->getCustomerByToken($this->customerToken) ?? 0;
            $this->purchasePoint = $this->wholeData["purchasePoint"] ?? "web";
            $this->paymentId = $this->wholeData['pamyent_id'] ?? "";
            $this->shippingArrivalDate = $this->wholeData['shipping_arrival_date'] ?? "";
            $this->deliveryTimeSlot = $this->wholeData['delivery_time_slot'] ?? "";
            $this->shippingArrivalComments = $this->wholeData['shipping_arrival_comments'] ?? "";
            if ($this->customerId > 0) {
