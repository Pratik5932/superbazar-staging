...
class Catalog extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    /**
    * \Webkul\MobikulCore\Block\Configurable
    */
    ...
    * @param \Webkul\MobikulCore\Block\Configurable                         $configurableBlock       cofigurableBlock
                ...
                if ($_filter->getIsFilterable() == 1) {
                    if (!empty($optionsCount[$option["value"]])) {
                        $data[] = [
                            ...
                            "label" => htmlspecialchars_decode($option["label"])
                        ];
                    }
                } else {
                    $data[] = [
    ...
    */
    public function getOneProductRelevantData($product, $storeId, $width, $customerId = 0)
    {
        ...
        if ($product->getTypeId() == "configurable") {
            ...
            # echo $configurableBlock->getJsonConfig();;exit;
            $eachProduct["configurableData"] = json_decode($configurableBlock->getJsonConfig());
            # echo $eachProduct["configurableData"];exit;
        } else {
        ...
        #mail("er.bharatmali@gmail.com","tets", $_REQUEST['address']);
        if($postcodeProdctPrice !== null){
        ...
        if($postcodeProdctPriceArray && is_array($postcodeProdctPriceArray) && !empty($postcodeProdctPriceArray)){
            ...
            if($zipCodePrice != null && !$specialPriceflag){
               # mail("er.bharatmali@gmail.com","tets111", $zipCodePrice);
                $eachProduct["postcode_prodct_price"]['price'] =$zipCodePrice;
                $eachProduct["price"] = $zipCodePrice;
                $eachProduct["formattedPrice"] = $this->stripTags($this->priceCurrency->format($zipCodePrice));
                ...
                #mail("er.bharatmali@gmail.com","tets2222", $zipCodePrice);
                $eachProduct["postcode_prodct_price"]['price'] =$specialprice;
                $eachProduct["price"] = $specialprice;
                $eachProduct["formattedPrice"] = $this->stripTags($this->priceCurrency->format($specialprice));

                $eachProduct["finalPrice"] = $specialprice;
                $eachProduct["formattedFinalPrice"] = $this->stripTags($this->priceCurrency->format($specialprice));;
            }
        }}
        $eachProduct["name"] = htmlspecialchars_decode($product->getName());
        #  mail("er.bharatmali@gmail.com","expiredate",htmlspecialchars_decode($product->getPreviousOrderExpiryDate());

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $productStockObj = $objectManager->get('Magento\CatalogInventory\Api\StockRegistryInterface')->getStockItem($product->getId());
        #mail("er.bharatmali@gmail.com","asdsa",$productStockObj->getIsInStock());
        if($product->getPreviousOrderExpiryDate() !== null){
        $date = str_replace('/', '-', $product->getPreviousOrderExpiryDate());
        $expireDate = date("d-M-Y", strtotime($date));
        }
        $eachProduct["is_instock"] =$productStockObj->getIsInStock();
        $eachProduct["previous_order_expiry_date"] = $product->getPreviousOrderExpiryDate();

        #  mail("er.bharatmali@gmail.com","expiredate",print_r( $eachProduct, true ));
        ...
        if (count($tier_price) > 0) {
            foreach ($tier_price as $key => $value) {
                ...
                $price = $value['price'];
                $formattedPrice =$this->priceFormat->currency(number_format($price, 2), true, false);
                $savePercentageFormat = ceil(100 - ( (100 / $product->getFinalPrice())* $value['price']) );
                //foreach (array_reverse($pirces) as $k => $v) {
                if ($key == "price") {
                    //$tp = number_format($value, 2, '.', '');
                    $eachProduct['tierPrice'] = "Buy $qty for <span class=\"price\">$formattedPrice</span> each";
                    $eachProduct['formattedTierPrice'] = $formattedPrice;
                }
                //  }
            }
        ...
        //  mail("er.bharatmali@gmail.com","tets", print_r( $eachProduct, true )); 
        return $eachProduct;
    }
    ...
    */
    protected function isProductNew($product)
    {
        ...
        if ($productNewFromDate && $productNewToDate) {
    ...
    */
    public function getPriceFilter($priceFilterModel, $storeId)
    {
        if ($this->getPriceRangeCalculation() == "improved") {
            $objectManager = \Magento\Framework\app\ObjectManager::getInstance();
            // $algorithmModel = $objectManager->create(\Magento\Catalog\Model\Layer\Filter\Price\Algorithm::class);
