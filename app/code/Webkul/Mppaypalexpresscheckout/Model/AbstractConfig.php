...
abstract class AbstractConfig implements ConfigInterface
{
    ...
    public function getValue($key, $storeId = null)
    {
        switch ($key) {
            case 'getDebugReplacePrivateDataKeys':
                return $this->_methodInstance->getDebugReplacePrivateDataKeys();
            default:
                $underscored = strtolower(
                    preg_replace('/(.)([A-Z])/', '$1_$2', $key)
                );
    ...
    protected function _prepareValue($key, $value)
    {
        ...
        if ($key == 'payment_action'
            ...
            && $this->_methodCode == self::METHOD_CODE
    ...
    /**
     * Check whether only Unilateral payments (Accelerated Boarding)
     * possible for Express method or not.
     *
     * @return bool
     */
    public function shouldUseUnilateralPayments()
    {
        return $this->getValue('business_account')
        && !$this->isWppApiAvailabe();
    }
    ...
    /**
     * Check whether method active in configuration and
     * supported for merchant country or not.
     *
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     */
    public function isMethodSupportedForCountry(
        $method = null,
        $countryCode = null
    ) {
        return true;
